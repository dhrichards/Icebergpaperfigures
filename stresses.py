...
filename = f'relaxation_spectral_level0.0height300.0Gc0.5dt50.0psicrit1.0l10.0cellfactor1.0gv_tol4.0_damagemodelAT2higher_3dd_.bp'
...
model = kr.base.Simulation(msh, kr.momentum.totalelastic.SemiLagrangian3D,
...
def viscosity(ε, n, eps=1e-14, A=1.0): 
    εe2 = ufl.inner(ε, ε) / 2 + eps
    return  A**(-1/n) * εe2**((1 - n) / (2 * n))/2

def viscosity_stress(τ, n, eps=1.e-14, A=1.0):
    # σD = dev3(σ)
    τe2 = ufl.inner(τ, τ) / 2 + eps
    return 0.5*τe2**((1 - n) / 2)/A
...
ν = model.params.ν.value
λ = ν/((1 + ν)*(1 - 2*ν))
μ = 1/(2*(1 + ν))
K = λ + 2*μ/3


model.params.T.value = -30

# η = mf.viscosity(ufl.dev(mf.tensor_2d_to_3d(mf.ε(model.momentum.vel))), n,0)

dεv = (ε3d(model.momentum.du) - model.momentum.dε_e)/model.params.dtstar



# σe = 2*model.momentum.ε_e + es.λoverμ(ν)*ufl.tr(model.momentum.ε_e)*ufl.Identity(2)
σe = 2*mf.dev3(model.momentum.ε_e)

σD3 = ufl.dev(mf.tensor_2d_to_3d(2*model.momentum.ε_e))

σe2 = 0.5*ufl.inner(ufl.dev(σe), ufl.dev(σe))
A = mf.rate_factor(model.params.T)/model.params.A0
η = 1/A/σe2

σv = 2*η*mf.dev3(dεv)

# σv = 2*η*mf.tensor_2d_to_3d(mf.εD(model.momentum.vel)) - model.momentum.p*ufl.Identity(3)
# τe = ufl.sqrt(0.5*ufl.inner(model.momentum.τ, model.momentum.τ) + 1.e-16)
...
#add line y=x
x = np.linspace(np.min(d2a(σv[0,1])), np.max(d2a(σv[0,1])), 100)
